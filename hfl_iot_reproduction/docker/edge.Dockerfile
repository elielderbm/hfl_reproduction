...
RUN pip install --no-cache-dir numpy==1.26.4 pandas==2.2.2 paho-mqtt==1.6.1     pyyaml==6.0.2 matplotlib==3.9.0 scikit-learn==1.5.1     tensorflow-cpu==2.15.0.post1

WORKDIR /workspace

# Edge services (TF not required, but included for simplicity)
COPY project /workspace/project
COPY config /workspace/config
RUN mkdir -p /workspace/logs /workspace/outputs /workspace/data
