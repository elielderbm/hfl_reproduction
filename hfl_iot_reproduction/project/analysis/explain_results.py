...
def _artifact_description(name: str) -> str:
    ...
    if name.startswith("iot_") and name.endswith("_acc.png"):
        return "IoT training accuracy per local round (from plot_curves.py)."
    if name.startswith("iot_") and name.endswith("_loss.png"):
        return "IoT training loss per local round (from plot_curves.py)."
...
def _build_iot_summary(iot_df: pd.DataFrame):
    ...
    for iot_id, sub in sorted(iot_df.groupby("iot"), key=lambda kv: str(kv[0])):
        ...
        acc_col = "train_acc" if "train_acc" in sub.columns else "val_acc"
        loss_col = "train_loss" if "train_loss" in sub.columns else "val_loss"
        acc = sub[acc_col].dropna() if acc_col in sub.columns else pd.Series(dtype=float)
        loss = sub[loss_col].dropna() if loss_col in sub.columns else pd.Series(dtype=float)
...
def _plot_interpretation_lines(iot_map: dict, edge_map: dict, cloud_summary: dict | None, artifact_names: list[str]) -> list[str]:
    ...
    for name in artifact_names:
        ...
        if m:
            ...
            lines.append(
                f"- `{name}`: evolution of IoT `{iot_id}` training accuracy across local rounds; "
                f"last={_fmt_float(info.get('last_acc'))}, best={_fmt_float(info.get('best_acc'))}."
            )
            continue

        m = re.match(r"^iot_(.+)_loss\.png$", name)
        if m:
            iot_id = m.group(1)
            info = iot_map.get(iot_id, {})
            lines.append(
                f"- `{name}`: evolution of IoT `{iot_id}` training loss; "
...
def generate_results_explanation(df: pd.DataFrame | None = None) -> Path:
    ...
    report_lines = [
        ...
        "- IoT behavior comes from `project/iot/device.py`: each device trains locally, reports `train_acc`/`train_loss` (and `val_*` when available), encrypts weights, and sends updates to its edge.",
    ...
    report_lines.extend(
        [
            ...
            "- Cloud logs include `global_acc`/`global_loss` when available; if missing, global quality should be inferred from IoT trajectories.",
