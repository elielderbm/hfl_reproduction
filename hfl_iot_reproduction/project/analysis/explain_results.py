...
def _artifact_description(name: str) -> str:
    ...
    if name.startswith("edge_") and "_ft_" in name:
        return "Edge fine-tuning metric (student on proxy data)."
...
def _build_edge_summary(edge_df: pd.DataFrame):
    ...
    for edge_id, sub in sorted(edge_df.groupby("edge"), key=lambda kv: str(kv[0])):
        ...
        ft_rmse = sub["edge_ft_rmse"].dropna() if "edge_ft_rmse" in sub.columns else pd.Series(dtype=float)
        ft_score = sub["edge_ft_score"].dropna() if "edge_ft_score" in sub.columns else pd.Series(dtype=float)
        ...
        ft_rmse_last = float(ft_rmse.iloc[-1]) if len(ft_rmse) > 0 else None
        ft_score_last = float(ft_score.iloc[-1]) if len(ft_score) > 0 else None
        ...
        by_edge[str(edge_id)] = {
            ...
            "q_mean": q_mean,
            "buf_mean": buf_mean,
            "edge_ft_rmse_last": ft_rmse_last,
            "edge_ft_score_last": ft_score_last,
        }
...
def generate_results_explanation(df: pd.DataFrame | None = None) -> Path:
    ...
    report_lines = [
        ...
        "- Cloud behavior comes from `project/cloud/server.py`: weighted edge fusion using `v_e = beta_cloud*p_e + (1-beta_cloud)*q_e`, per-target global models, and optional teacher training on proxy data.",
